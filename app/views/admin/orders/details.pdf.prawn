...
if @notary!=nil
pdf.text "               Notary:  " , :size => 9, :style => :bold, :at =>[10,488]
pdf.text "   #{@order.notary.first_name} #{@order.notary.last_name}" , :size => 8, :at =>[75,488]

if @order.doc_delivery_address_option==nil or @order.doc_delivery_address_option=="weekday"

pdf.text "    Doc Delivery" , :size => 9, :style => :bold, :at =>[10,476]
pdf.text "   #{@order.notary.weekday_deliver_address}" , :size => 8, :at =>[75,476]
pdf.text "           Address :" , :size => 9, :style => :bold, :at =>[10,464]
pdf.text "   #{@order.notary.weekday_deliver_city}, #{@order.notary.weekday_deliver_state} #{@order.notary.weekday_deliver_zip_code}" , :size => 8, :at =>[75,464]
...
pdf.text "    Doc Delivery" , :size => 9, :style => :bold, :at =>[10,476]
pdf.text "   #{@order.notary.weekend_deliver_address}" , :size => 8, :at =>[75,476]
pdf.text "           Address :" , :size => 9, :style => :bold, :at =>[10,464]
if @order.notary.weekend_deliver_city.size!=0
pdf.text "   #{@order.notary.weekend_deliver_city}, #{@order.notary.weekend_deliver_state} #{@order.notary.weekend_deliver_zip_code}" , :size => 8, :at =>[75,464]
else
pdf.text "   #{@order.notary.weekend_deliver_city} #{@order.notary.weekend_deliver_state} #{@order.notary.weekend_deliver_zip_code}" , :size => 8, :at =>[75,464]
end
...
pdf.text "    Doc Delivery" , :size => 9, :style => :bold, :at =>[10,476]
pdf.text "   #{@order.notary.billing_deliver_address}" , :size => 8, :at =>[75,476]
pdf.text "           Address :" , :size => 9, :style => :bold, :at =>[10,464]
pdf.text "   #{@order.notary.billing_deliver_city}, #{@order.notary.billing_deliver_state} #{@order.notary.billing_deliver_zip_code}" , :size => 8, :at =>[75,464]

end


pdf.text "  NotaryFly ID #:  " , :size => 9, :style => :bold, :at =>[10,452]
pdf.text "   #{@order.notary.id}" , :size => 8, :at =>[75,452]
pdf.text "      Signing Fee:  " , :size => 9, :style => :bold, :at =>[10,440]

if @order.order_notary_fee!=nil
pdf.text "   $#{@order.order_notary_fee}" , :size => 8, :at =>[75,440]
else
pdf.text "   Not Provided" , :size => 8, :at =>[75,440]
end

pdf.text "                 Day #:  " , :size => 9, :style => :bold, :at =>[10,428]

if @order.notary.day_phone.size!=0
pdf.text "   #{@order.notary.day_phone}" , :size => 8, :at =>[75,428]
else
pdf.text "   Not Provided" , :size => 8, :at =>[75,428]
end

pdf.text "         Evening #:   " , :size => 9, :style => :bold, :at =>[10,416]

if @order.notary.evening_phone.size!=0
pdf.text "   #{@order.notary.evening_phone}" , :size => 8, :at =>[75,416]
else
pdf.text "   Not Provided" , :size => 8, :at =>[75,416]
end

pdf.text "           Mobile #:   " , :size => 9, :style => :bold, :at =>[10,404]
if @order.notary.mobile_phone.size!=0
pdf.text "   #{@order.notary.mobile_phone}" , :size => 8, :at =>[75,404]
else
pdf.text "   Not Provided" , :size => 8, :at =>[75,404]
end

pdf.text "              Work #:   " , :size => 9, :style => :bold, :at =>[10,392]

if @order.notary.work_phone.size!=0
pdf.text "   #{@order.notary.work_phone}" , :size => 8, :at =>[75,392]
else
pdf.text "   Not Provided" , :size => 8, :at =>[75,392]
end

pdf.text "                Email:   " , :size => 9, :style => :bold, :at =>[10,380]
pdf.text "   #{@order.notary.user.email}" , :size => 8, :at =>[75,380]
end
...
pdf.text "Notary Payment Address:", :size => 10, :style => :bold, :at => [0,340]
if @notary!=nil
pdf.text "#{@order.notary.billing_payable_to}", :size => 9, :at => [10,320]
pdf.text "#{@order.notary.billing_deliver_address}", :size => 9, :at => [10,308]
pdf.text "#{@order.notary.billing_deliver_city}, #{@order.notary.billing_deliver_state} #{@order.notary.billing_deliver_zip_code}", :size => 9, :at => [10,296]
else
