class Client::MessagesController < ApplicationController
  ...
  def create
    ...
    @message.recipient_id = @order.notary.user_id

    if @message.save
      @notary = Notary.find(@order.notary_id)
      client_email = User.find(@notary.user_id)
      client_email = client_email.email
     Notifier.deliver_notary_new_message_recipient(@order,client_email) #client_email
